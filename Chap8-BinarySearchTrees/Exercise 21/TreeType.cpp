...
void TreeType::LeftGreatest_Recursive(TreeNode*& ptr) { // 최솟값의 오른쪽에 rightchild가 있을 경우 모두 삭제 후 가장 rightchild 유지
    TreeNode* leastPtr = ptr;
    if (ptr->right != NULL)
        LeftGreatest_Recursive(ptr->right);
    else {
        while (leastPtr != ptr) {
           DeleteItem(leastPtr->info);
           leastPtr = leastPtr->right;
        }
        /*DeleteItem(leastPtr->info);*/
    }
}
...
TreeNode* TreeType::PtrToSuccessor_Recursive(TreeNode* tree)
{
    if (tree->left != NULL)
        ...
        ItemType temp = tempPtr->info;
        LeftGreatest_Recursive(tree);
        DeleteItem(temp);
...
void TreeType::CheckPTS_Recur() {
    ofstream fStream;
    TreeNode* treePtr = PtrToSuccessor_Recursive(root);

    fStream.open("answer.txt");
    Print(fStream);
    fStream.close();

    cout << treePtr->info << endl;
}
...
void TreeType::CheckPTS_NonRecur() {
    ofstream fStream;
    TreeNode* treePtr = PtrToSuccessor_NonRecursive(root);
